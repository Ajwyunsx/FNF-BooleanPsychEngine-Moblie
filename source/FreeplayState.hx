...
class FreeplayState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		//trace("" + lerpScore + " " + lerpRating + " " +lerpRatingFC);

		lerpRatingFC= Highscore.getRatingFC(songs[curSelected].songName,curDifficulty);
		...
		if(lerpRatingFC == "")
			...
			scoreText.text = 'PERSONAL BEST: ' + lerpScore + ' (' + ratingSplit.join('.') + '%) - ' + lerpRatingFC;
		positionHighscore();
		...
		if (FlxG.keys.justPressed.F2 #if android || _virtualpad.buttonZ.pressed && FlxG.android.justReleased.BACK #end)
			{
				persistentUpdate = false;
				destroyFreeplayVocals();
				LoadingReplayState.sName = songs[curSelected].songName;
				MusicBeatState.switchState(new LoadingReplayState());
				//LoadingReplayState.setMod(songs[curSelected].folder);
			}
