...
class NoteOffsetState extends MusicBeatState
{
	...
	var sickComboNums:FlxSpriteGroup;
	...
	override public function create()
	{
		...
		sickComboNums = new FlxSpriteGroup();
		sickComboNums.cameras = [camHUD];
		add(sickComboNums);

		var seperatedScore:Array<Int> = [];
		var sickSeperatedScore:Array<Int> = [];
		for (i in 0...3)
		{
		...
		for (i in 0...3)
		{
			sickSeperatedScore.push(FlxG.random.int(0, 9));
		}
		...
		trace(sickSeperatedScore);

		for (i in sickSeperatedScore)
		{
			if(i != 0 )ignore0=false;
			if(ignore0)continue;
			var sickScore:FlxSprite = new FlxSprite(30 * daLoop2).loadGraphic(Paths.image('num' + i));
			...
			sickScore.antialiasing = ClientPrefs.globalAntialiasing;
			sickComboNums.add(sickScore);
			daLoop2++;
		}
	...
	var holdingObjectType2:Bool = false;
	...
	override public function update(elapsed:Float)
	{
		...
		if(onComboMenu)
		{
			...
			if (FlxG.mouse.justPressed)
			{
				...
				else if (startMousePos.x - sickComboNums.x >= 0 && startMousePos.x - sickComboNums.x <= sickComboNums.width &&
					startMousePos.y - sickComboNums.y >= 0 && startMousePos.y - sickComboNums.y <= sickComboNums.height)
				{
					holdingObjectType = true;
					holdingObjectType2 = true;
			...
			if(holdingObjectType != null)
			{
				if(FlxG.mouse.justMoved)
				{
					...
					if(addNum==2)addNum = holdingObjectType2 ? 4 : 2;
	...
	function repositionCombo()
	{
		...
		sickComboNums.screenCenter();
		sickComboNums.x = coolText.x + 220 + ClientPrefs.comboOffset[4];
		sickComboNums.y -= 60 + ClientPrefs.comboOffset[5];
		reloadTexts();
	}
	...
	function reloadTexts()
	{
		for (i in 0...dumbTexts.length)
		{
			switch(i)
			{
				...
				case 5: dumbTexts.members[i].text = '\n[' + ClientPrefs.comboOffset[4] + ', ' + ClientPrefs.comboOffset[5] + ']';
			}
		}
	}
