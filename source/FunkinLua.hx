package;
/*改动说明：颜色部分Std.parseInt("0xff" + color);
	均改为：0xFF000000 + Std.parseInt("0x" + color);
	开头不是0xFF的单独截出来再*0x1000000

	2024.11.29修正：对于形如0xffffff的输入，转换为ffffff。
		getColorFromHex若传入的是六位ffffff值，则接上前缀0xff而非0x。

		避免实际转换变成0xff00ffff导致角色“变蓝”

	貌似原写法会造成溢出截断导致值恒为2147483647(7FFF FFFF)。
	还是比较麻烦的
	*/
...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String) {
			if(targetColor.length == 8 && targetColor.startsWith('0x')){
				targetColor = targetColor.substr(2,8);
			}
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				var color:Int = Std.parseInt(targetColor);
				if(!targetColor.startsWith('0x')) color = 0xFF000000 + Std.parseInt('0x' + targetColor);
				else color = Std.parseInt(targetColor.substr(0,4))*0x1000000 + Std.parseInt('0x' + targetColor.substr(4,targetColor.length));
		...
		Lua_helper.add_callback(lua, "noteTweenX", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String) {
			...
			//I edit this due to the mstiming above the note
			if(ClientPrefs.openMsTiming){
				if(note == 4)
					var tw = FlxTween.tween(PlayState.instance.msTiming0,{x:value},duration);
				if(note == 5)
					var tw = FlxTween.tween(PlayState.instance.msTiming1,{x:value},duration);
				if(note == 6)
					var tw = FlxTween.tween(PlayState.instance.msTiming2,{x:value},duration);
				if(note == 7)
					var tw = FlxTween.tween(PlayState.instance.msTiming3,{x:value},duration);
			}
		...
		Lua_helper.add_callback(lua, "getColorFromHex", function(color:String) {
			if(!color.startsWith('0x')) {
				if(color.length == 6)color = '0xff' + color;
					else color = '0x' + color;
			}

			return Std.parseInt(color.substr(0,4))*0x1000000 + Std.parseInt('0x' +color.substr(4,color.length));
		});
		...
		Lua_helper.add_callback(lua, "cameraFlash", function(camera:String, color:String, duration:Float,forced:Bool) {
			...
			if(!color.startsWith('0x')) colorNum = 0xFF000000 + Std.parseInt('0x' + color);
			else colorNum = Std.parseInt(color.substr(0,4))*0x1000000 + Std.parseInt('0x' +color.substr(4,color.length));
		...
		Lua_helper.add_callback(lua, "setHealthBarColors", function(leftHex:String, rightHex:String) {
			...
			if(leftHex.length == 8 && leftHex.startsWith('0x')){
				leftHex = leftHex.substr(2,8);
			}
			if(!leftHex.startsWith('0x')) left = 0xFF000000 + Std.parseInt('0x' + leftHex);
			else left = Std.parseInt(leftHex.substr(0,4))*0x1000000 + Std.parseInt('0x' +leftHex.substr(4,leftHex.length));
			var right:FlxColor = Std.parseInt(rightHex);
			if(rightHex.length == 8 && rightHex.startsWith('0x')){
				rightHex = leftHex.substr(2,8);
			}
			if(!rightHex.startsWith('0x')) right = 0xFF000000 + Std.parseInt('0x' + rightHex);
			else right = Std.parseInt(rightHex.substr(0,4))*0x1000000 + Std.parseInt('0x' +rightHex.substr(4,rightHex.length));
